...
async function updateLockfile(npmrc, pkg, context) {
	...
	const lockfile = join(pkg.location, "package-lock.json");
 ...
 * @returns {Promise<void>}
 */
export default async (npmrc, pluginConfig, context) => {
	const {
		cwd,
		nextRelease: { version },
		logger,
	} = context;
	const basePath = pluginConfig.pkgRoot ? resolve(cwd, pluginConfig.pkgRoot) : cwd;
	const rootPkg = new Package(readJson(join(basePath, "package.json")), basePath);
